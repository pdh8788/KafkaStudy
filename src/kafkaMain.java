public class kafkaMain {
    ...
    /**
     * 카프카는 대규모 메시지 데이터를 빠르게 처리하도록 개발된 메시징 플랫폼이다.
     *
     * 일반적인 통신 모델은 통신에 참여하는 개체가 많아질수록 서로 일일이 다 연결을하고 데이터를 전송해야 하기 때문에 확장성이 좋지 않음.
     * 프로듀서가 데이터와 수신처 id를 포함시켜 메시징 시스템으로 보내면 교환기가 수신처 id값을 확인한 다음 수신처 id값에 맞는 큐에 데이터를 전달합니다.
     * 컨슈머는 자신들의 큐를 모니터링 하고 있다가 큐에 메시지가 전달되면 이 값을 가져옵니다.
     * 이렇게 할 경우 혹시나 개체가 하나 빠지거나 수신 불능 상태가 되었을 떄에도, 메시징 시스템만 살아 있으면 프로듀서에서 전달된 메시지가 유실되지 않습니다.
     * 이 메시지는 불능 상태의 개체가 다시 회복되면 언제든지 가져갈 수 있습니다.그리고 각각의 개체가 다대다 통신을 하는것이 아니라 메시징 시스템을 중심으로
     * 연결되기 때문에 확장성이 용이합니다.
     *
     * 카프카는 메시징 시스템이 지닌 성능의 단점을 극복하기 위해, 메시지 교환 전달의 신뢰성 관리를 프로듀서와 컨슈머 쪽으로 넘기고, 부하가 많이 걸리는
     * 교환기 기능 역시 컨슈머가 만들수 있게 함으로써 메시징 시스템 내에서의 작업량을 줄이고 이렇게 절약한 작업량을 메시징 전달 성능에 집중시켜서
     * 고성능 메시징 시스템을 만들어 냈습니다.
     *
     * 카프카의 특징
     * 1. 메시지를 보내는 역햘과 받는 역할이 완벽하게 분리된 펍/섭 방식을 적용.
     * 2. 멀티 프로듀서, 멀티 컨슈머
     *  - 하나의 프로듀서가 하나의 토픽에만 메시지를 보내는 것이 아니라, 하나 또는 하나 이상의 토픽으로 메시지를 보낼수 있습니다.
     *  - 컨슈머는 역시 하나의 토픽에서만 가져오는게 아니라, 하나 또는 하나 이상의 토픽으로부터 메시지를 가져올 수 있습니다.
     *
     * 3. 디스크에 미시지 저장
     *   - 카프카는 컨슈머가 메시지를 일겅 가더라도 정해져 있는 보관 주기 동안 디스크에 메시지를 저장해 둡니다. 트래픽이 일시적으로
     *   폭주해 컨슈머의 처리가 늦어지더라도 카프카의 디스크에 안전하게 보관되어 있기 때문에, 컨슈머는 메시지 손실 없이 메시지를 가져갈 수 있습니다.
     *   - 컨슈머에 버그가 있어 오류가 발생 했다면, 컨슈머를 잠시 중단하고 버그를 찾아 해결한 후 컨슈머를 다시 실행할 수 있습니다.
     *
     * 4. 확장성이 높음
     *
     * 5. 높은 성능
     *
     * 1. 카프카의 구성
     *  - 카프카는 크게 프로듀서, (카프카(브로커) // 카프카 애플리케이션이 설치되어 있는 서버 또는 노드를 말합니다,) 컨슈머, 주키퍼로 구성되어 있다.
     *  1) 주키퍼
     ...
     *  과반수로 정하기 때문에 주키퍼의 서버는 홀수로 구성되어야 한다.
     *
     *  1-1) 주키퍼 환경설정
     ...
     *  dataDir = /data // 주키퍼의 트랜잭션 로그와 스냅샷이 저장되는 데이터 저장 됩니다.
     ...
     *  다만, 분산 애플리케이션이라는 측면에서 보면 주키퍼와 동일하지만 클러스터가 운영되는 방식은 다릅니다.
     *  과반수 방식으로 운영되어 홀수로 서버를 구성해야 하는 주키퍼와난 다르게 카프카 클러스터는 홀수 운영 서버를 하지 않아도 됩니다.
     *
     *  주키퍼와 카프카는 동일한 서버가 아닌 별도의 주키퍼 서버와 별도의 카프카 서버로 구성하기를 권장합니다.
     *
     ...
     *  -> 주키퍼 정보를 입력할 때에는 주키퍼 앙상블 서버 리스트를 모두 입력해야 합니다.
     *
     ...
     *
     *  2-3) 주키퍼 지노드를 이용한 카프카 정보 확인
     *  카프카와 주키퍼가 잘 설치되고 연결되었는지를 주키퍼에 CLI에 직접 접속해서 확인할 수 있습니다.
     *  /usr/local/zookeeper/bin/zkCli.sh CLI 접속
     *  ls /peter-kafka/brokers/ids -> (출력: 1,2,3) broker.id 번호 리스트를 확인할 수 있음
     *  
     * 
     *  3. 카프카 디자인
     *  3-1) 분산 시스템
